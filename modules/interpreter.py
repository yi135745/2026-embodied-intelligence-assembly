"""图片语义解释能力：任务一描述与任务二任务卡提取。

对外接口：
    Interpreter.identify_image(image) -> str  识别图片并返回可直接播报的文本。
"""
...
import config
from drivers.dashscope import post_json
...
class Interpreter:
    ...
    def identify_image(self, image) -> str:
        ...
        result = post_json(
            self.api_url, payload, api_key, self.api_timeout, operation="模型接口"
        )
    ...
    def parse_task2_card(self, image, output_dir=None) -> list:
        """通过视觉API识别任务卡2并解析JSON；业务规则由任务层校验。"""
        image_path = image if isinstance(image, Path) else Path(image)
        api_key = config.DASHSCOPE_API_KEY
        if not api_key:
            raise RuntimeError("请先在 config.py 中配置 DASHSCOPE_API_KEY。")
        ...
        try:
            result = post_json(
                self.api_url, payload, api_key, self.api_timeout, operation="任务卡2 API"
            )
        except Exception as exc:
        ...
        if log_path is not None:
            with log_path.open("a", encoding="utf-8") as stream:
                stream.write("\n[PARSED_JSON]\n")
                stream.write(json.dumps(steps, ensure_ascii=False, indent=2))
                stream.write("\n")
        return steps
