...
**内容**
- 核心：非结构化任务卡 → 结构化任务信息。
- 任务一：图像 → Qwen Vision → 场景内容（`identify_image()`），prompt 强制输出「任务卡1中有A、B、C。」单行格式。
- 任务二：任务卡 → Qwen → 六步 JSON（`parse_task2_card()`），**temperature=0** 确定性输出。
- 程序强校验：恰好 6 步 → 颜色名合法 → 方块六色完整覆盖 → 托盘六色完整覆盖，任一失败抛错。
- 细节：上传前图片压缩（任务一最长边 1280 + JPEG 85；任务二最长边 2560 保小字）。
- 原始返回写 `task2_llm_*.txt` 留痕。
- 核心句：**大模型负责语义理解，确定性程序负责结果约束与执行。**

**📍 引用**
- 任务一 prompt：[config.py:150-154](config.py#L150-L154)
...
- 任务二 prompt 与上传分辨率：[config.py:239-243](config.py#L239-L243)、[config.py:217](config.py#L217)
