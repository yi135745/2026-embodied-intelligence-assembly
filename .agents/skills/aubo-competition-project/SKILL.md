...
- 方块区与托盘区共用唯一平面矩阵 `resources/visionmaster_task2_calibration.xml`；XY 拍照位可以不同，相机到各自观测平面的有效高度及姿态必须一致。`data/task2_offsets_v2.json` 保存一次人工物理锚点和像素偏心观测，运行时由当前矩阵派生唯一相机—吸盘偏移供两区共用。
- Z 从 `config.py` 读取，抓放姿态从当前拍照位读取；XY 偏移文件不拥有 Z，也不能替代位姿文件。
- 正式矩阵、质量绑定和完整观测报告是一条版本链；人工物理锚点是独立原始观测，不绑定矩阵先后顺序。启用新矩阵后必须重新派生毫米偏移与像素旋转偏心并做落点验收，但不要求重做人工粗校。
