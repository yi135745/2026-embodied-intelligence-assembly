...
**当前状态：功能代码已接入，37 项离线回归通过，AI 盒子健康检查通过；国赛实物的颜色、角度、高度、语音和整机精度尚未完成验收。软件测试通过不等于整机已调通。**
...
| 回归与文档 | 原省赛测试/说明 | 新增计划模块、语音协议回归、盒子独立测试及说明 | 当前 37 项离线用例，不接真实硬件或外部 API |
...
通过标准：37 项全部 `OK`。覆盖动作规则、九色合成图、角度/高度、运动顺序模拟、失败不提交、主流程完成判定及语音异常；不连接盒子、相机、机器人或视觉 API，也不加载本地语音模型。
...
- [x] 协议、九色合成图、方向/高度、状态及语音离线回归：37 项通过。
